...
export const serializeProofData = (
  proofData: { proof: `0x${string}`; publicInputs: any[] },
  contractAddress?: string,
  voteChoice?: boolean,
...
): { proof: `0x${string}`; publicInputs: any[] } => {
  return {
    proof: serializedData.proof as `0x${string}`,
...
export const loadProofFromLocalStorage = (
  contractAddress?: string,
  userAddress?: string,
): { proof: `0x${string}`; publicInputs: any[] } | null => {
  try {
    const storageKey = getStorageKey(contractAddress, userAddress);
