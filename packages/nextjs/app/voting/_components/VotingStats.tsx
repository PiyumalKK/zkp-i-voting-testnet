    ...
    <div className="glass-card p-6 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-center">
        <div className="rounded-xl border border-success/20 bg-success/5 p-4 transition-all hover:border-success/40">
          <div className="text-xs font-semibold uppercase tracking-wider text-success/70">Yes</div>
          <div className="text-2xl font-black text-success mt-1">{yes.toString()}</div>
          <div className="text-xs font-medium text-success/60 mt-0.5">{yesPercentage.toFixed(1)}%</div>
        </div>
        <div className="rounded-xl border border-error/20 bg-error/5 p-4 transition-all hover:border-error/40">
          <div className="text-xs font-semibold uppercase tracking-wider text-error/70">No</div>
          <div className="text-2xl font-black text-error mt-1">{no.toString()}</div>
          <div className="text-xs font-medium text-error/60 mt-0.5">{noPercentage.toFixed(1)}%</div>
        </div>
      </div>
      {totalVotes > 0n && (
        <div className="w-full bg-base-content/5 rounded-full h-2.5 overflow-hidden flex">
          <div className="bg-gradient-to-r from-success to-success/80 h-2.5 rounded-l-full transition-all duration-500" style={{ width: `${yesPercentage}%` }} />
          <div className="bg-gradient-to-r from-error/80 to-error h-2.5 rounded-r-full transition-all duration-500" style={{ width: `${noPercentage}%` }} />
        </div>
      )}
    </div>
  );
